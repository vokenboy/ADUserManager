...
public class GroupService : ActiveDirectoryBase, IDirectoryGroupService
{
    ...
    // IDirectoryGroupService async implementations
    public async Task<List<ADGroupModel>> SearchGroupsAsync(string filter)
    {
        return await Task.Run(() => SearchGroups(filter));
    }
